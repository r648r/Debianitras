...
wbm(){
  ...
  cat wb.txt | wc -l
  echo
  echo "cat wb.txt | grep -Eo '\.(xls|xml|xlsx|json|pdf|sql|doc|docx|pptx|txt|zip|tgz|bak|7z|rar|log|cache|secret|db|backup|yml|gz|config|csv|yaml|md|md5|exe|dll|bin|ini|bat|sh|tar|deb|rpm|iso|img|apk|msi|dmg|tmp|crt|pem|key|pub|asc|env|passwd|htpasswd|htaccess|keytab|csr|pfx|ppk)\$'"
  echo "cat wb.txt | sort -u | uro | grep -Eo '\.(xls|xml|xlsx|json|
